...
public class OpenMeteoPlugin
{
    ...
    [KernelFunction, Description("Gets current weather conditions from a latitude and longitude. This should not be used for predicting weather.")]
    ...
    [KernelFunction, Description("Gets current weather conditions from the user's default location. This should not be used for predicting weather.")]
    public async Task<string> GetWeatherForDefaultLocation()
    {
        string latLong = "41.382798,-82.643301"; // Kalahari Resort in Sandusky, OH
        return await GetCurrentWeatherFromLatLong(latLong);
    }
    ...
    */
    ...
    [KernelFunction, Description("Gets predicted weather conditions from the user's default location. This should not be used for predicting weather.")]
    public async Task<string> GetForecastForDefaultLocation([Description("A string representing a calendar date such as 9/10/80 or 2023-11-13 or today or tomorrow")] string dateStr)
    {
        string latLong = "41.382798,-82.643301"; // Kalahari Resort in Sandusky, OH
        return await GetDailyWeatherFromLatLong(latLong, dateStr);
    }

    [KernelFunction, Description("Gets weather information from a latitude and longitude for a given date")]
